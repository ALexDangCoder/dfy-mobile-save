...
jboolean JNICALL Java_wallet_core_jni_PublicKey_verifySchnorr(JNIEnv *env, jobject thisObject, jbyteArray signature, jbyteArray message) {
    jclass thisClass = (*env)->GetObjectClass(env, thisObject);
    jfieldID handleFieldID = (*env)->GetFieldID(env, thisClass, "nativeHandle", "J");
    struct TWPublicKey *instance = (struct TWPublicKey *) (*env)->GetLongField(env, thisObject, handleFieldID);
    ...
    TWData *signatureData = TWDataCreateWithJByteArray(env, signature);
    TWData *messageData = TWDataCreateWithJByteArray(env, message);
    jboolean resultValue = (jboolean) TWPublicKeyVerifySchnorr(instance, signatureData, messageData);
