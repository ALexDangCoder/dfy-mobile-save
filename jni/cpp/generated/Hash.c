...
jbyteArray JNICALL Java_wallet_core_jni_Hash_xxhash64(JNIEnv *env, jclass thisClass, jbyteArray data, jlong seed) {
    TWData *dataData = TWDataCreateWithJByteArray(env, data);
    jbyteArray result = NULL;
    TWData *resultData = TWHashXXHash64(dataData, seed);
    result = TWDataJByteArray(resultData, env);
    TWDataDelete(dataData);
    return result;
}

jbyteArray JNICALL Java_wallet_core_jni_Hash_twoXXHash64Concat(JNIEnv *env, jclass thisClass, jbyteArray data) {
    TWData *dataData = TWDataCreateWithJByteArray(env, data);
    jbyteArray result = NULL;
    TWData *resultData = TWHashTwoXXHash64Concat(dataData);
    result = TWDataJByteArray(resultData, env);
    TWDataDelete(dataData);
    return result;
}
